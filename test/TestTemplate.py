...
import unittest
from ExampleCalculationService.pvsystemservice import CalculationServicePVSystem
...
def simulator_environment_e_connection():
    return SimulatorConfiguration("PVInstallation", ["31206366-860f-4070-a163-21af451efe1c"], "Mock-PVInstallation", "127.0.0.1", BROKER_TEST_PORT, "test-id", SIMULATION_DURATION_IN_SECONDS, START_DATE_TIME, "test-host", "test-port", "test-username", "test-password", "test-database-name", h.HelicsLogLevel.DEBUG, ["PVInstallation", "EConnection"])
...
class Test(unittest.TestCase):
    ...
    def test_example(self):
        ...
        pv_dispatch_params = {}
        pv_dispatch_params["solar_irradiance"] = [0.8, 0.9]
        service.init_calculation_service(None)

        # Execute
        ret_val = service.predict_solar_power(pv_dispatch_params, datetime(2024,1,1), TimeStepInformation(1,2), "test-id", None)
